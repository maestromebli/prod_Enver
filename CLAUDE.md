...
- **`roles.js`** — ролі, `OPERATOR_STAGES` (4 етапи: cutting, edging, drilling, assembly), `DEFAULT_PERMISSIONS`.
...
- **Панелі цеху → Android** — порізка, крайкування, присадка, збірка (`operator.html`, `operator-*.js`, `part-scan.js`). Android APK — WebView над `/operator.html`; веб-зміни підтягуються після деплою через `__enverCheckForUpdates`. При зміні native API синхронізуй `operator-native.js` ↔ `EnverJsBridge.kt` і `npm run build:android-client`. Деталі: `.cursor/rules/workshop-android-sync.mdc`.
- **Audit functions** — async; після зміни позиції/замовлення `await logXxx(...)`.
- **Env**: `DATABASE_URL`, `DATABASE_URL_MIGRATIONS`, `UPLOADS_DIR`, `PORT`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `SESSION_SECRET`, `DOMAIN`, `ADMIN_DEFAULT_PASSWORD`. У production `SESSION_SECRET=enver-dev-secret` — fail-fast у `assertProductionSafety()`; `ADMIN_DEFAULT_PASSWORD=admin` блокує лише seed.
