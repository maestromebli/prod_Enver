...
| **Клієнти** | Windows (Electron), iPad/iPhone (Web Clip через `.mobileconfig`) |
...
### Windows (ENVER Operator)

1. Розпакуйте `operator-clients/windows/` або готовий `enver-operator-windows.zip`.
2. У `config.json` вкажіть URL ENVER OS:

```json
{
  "serverUrl": "https://enver.example.com",
  "kioskExitPassword": "1111",
  "autoLaunch": true
}
```

3. Запустіть `ENVER Operator.exe` — відкриється `/operator.html`.

### iPad / iPhone

1. У ENVER OS: **Налаштування → Клієнти** → завантажити профіль iPad.
2. Встановити профіль → іконка ENVER на домашньому екрані.
3. Увійти як оператор.
