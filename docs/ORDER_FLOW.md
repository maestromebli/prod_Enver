...
4. **Конструктив** — пакет (XLS/B3D/GLB) + опційно legacy-файл; pipeline: parse → review → procurement → ЧПК. Legacy-файл і legacy ШІ — лише якщо пакета ще немає.
5. **ШІ** — legacy-аналіз файлу (без пакета) або аналіз пакета; рекомендовані виробничі задачі.
6. **Закупівля / фінанси** — з пакета; при статусі `received` створюються фінансові записи.
7. **Виробництво** — handoff по етапах (порізка → крайкування → присадка → збірка → готово до встановлення).
