...
Root: `npm audit` показує 3 (root), 2 (server), 1 (client) vulnerability. Потребує окремого оновлення залежностей без поломки prod.
...
Не блокує збірку. Дрібне прибирання імпортів у наступному PR.
...
`part-viewer.js` ~783 kB. Рекомендація: code-splitting, не блокер.

### BUG-015 — access_token у query (design risk)

Severity: Low  
Area: security  
Status: needs-review

Компроміс для SSE/img/EventSource. Розглянути cookie-сесію для file GET у майбутньому.
...
`server/test/integration/*.test.mjs` skip без `INTEGRATION_TEST=1`. Локально не запускались у цьому аудиті — не баг коду.
